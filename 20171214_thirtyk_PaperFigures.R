...
labs <- fread(paste0(dir, "all_labs.csv"),
              header=TRUE,sep=',',stringsAsFactors=FALSE)
# cleaned 30K file <- generated by 20170908_thirtyk.R
corDf <- read.csv(paste0(dir, "/20170905_Cleaned_joined_30k_labs_vitals.csv"),
                   header=TRUE,sep=',',stringsAsFactors=FALSE)
# clean corDf *remove outliers*
...
# clean iPOPcorDf *remove outliers*
iPOPcorDf[, -c(1,2)] <- apply(iPOPcorDf[, -c(1,2)], 2, remove_outliers)

#################################
#  Figure 2C  - see Lukasz code #
#################################
#load-data.R - probably repetitive with what is already embedded in this script
# population-30k.R - Script to create ranked list of clinical laboratory tests by the correlation coefficients between observed and predicted values
# population-models.R - Script to compare different models for predicting lab tests from 30k vitals or iPOP wearables data

#########################################################
#  Figure 2D  - update with new models (Random Forest?) #
#########################################################
