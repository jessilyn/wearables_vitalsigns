...
allClin <- c("A1C","AG","ALB","ALCRU","ALKP","ALT","AST","BASO",
             ...
             "CR","EGFR","EOS","EOSAB","GLOB","GLU_byMeter",
...
#create freq. table of the results for a clinical test
table(labs$LAB_NAME[WBC])
...
# write.csv(clinSub,
#           paste0("/Users/Home/Desktop/Ongoing_Projects/",
#                  "Jessie_MobilizeCenter/Datasets/Clinical/",
#                  "SECURE_lab_results_new_18_JUL_2016_subset.csv"),
#           row.names=FALSE)
...
describe(vitals$Pulse); hist(vitals$Pulse)
describe(vitals$Temp); hist(vitals$Temp)

#Create freq. table
table(vitals$Pulse)
table(vitals$Temp)

#hist(winsor(vitals$Temp, trim = 0.01))
#describe(winsor(vitals$Temp, trim = 0.01))

#Transform to normal (in this case sqrt pulse; winsor Temp)
#describe(winsor(vitals$Pulse, trim = 0.01))
#vitals$Pulse <- winsor(vitals$Pulse, trim = 0.01)
#describe(winsor(vitals$Temp, trim = 0.01))
#vitals$Temp <- winsor(vitals$Temp, trim = 0.01)
...
#Remove columns with fewer than 3 observations

#corDf <- corDf[,-c(which(apply(
#  corDf,2,function(x) sum(!is.na(x)))<3))]
...
df <- merge(labs[,c("ANON_ID","Date","GROUP_LAB_NAME","LAB_NAME","ORD_VALUE")],
               vitals[,c("ANON_ID","Date","TEMPERATURE","PULSE")],
               by=c("ANON_ID","Date"))
...
for (i in 1:length(allClin)){
  p[i]<-lm(corDf$ORD_VALUE ~ corDF$PULSE + corDF$TEMPERATURE)
}
