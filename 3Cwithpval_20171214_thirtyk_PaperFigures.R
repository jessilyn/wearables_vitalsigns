...
for (i in 1:cv.runs){ #50 fold cross validation (10% test set; 90% training set)
  ...
  for (nm in nms){ # for each of the 50 clinical lab tests
    ...
    for (k in 1:length(models)){
      ...
      name.rsq <- paste("model.mean.rsq", k, sep = ".")
      models.corr.coefs <- rbind(models.corr.coefs,
                                 c(model = name.rsq, cv.step = i, test = nm, corr.coef = r.pred, sqrt.pct.var = sqrt.pct.var, numTestObs = numTestObs, numTrainObs = numTrainObs, pval = p.val))
    }
  }
}


write.table(models.corr.coefs, "/Users/jessilyn/Desktop/framework_paper/Figure3/Fig3C/20180801_model_compare_30k_withDemog.csv",row.names=FALSE,col.names=TRUE, sep=",")
