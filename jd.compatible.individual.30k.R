...
top = c("CR","HSCRP","NEUTAB","NEUT","LYM","RDW","ALB", "AG","PLT","TGL")
  
### REPLACE EVERYTHING BELOW WITH CORDF bs it is the cleaned version of vitals
#Reformat dates
vitals$CONTACT_DATE <- format(
  as.Date(vitals$CONTACT_DATE, "%d-%b-%Y"), "%Y-%m-%d")

labs.vitals = merge(data.frame(corDf), data.frame(vitals), by.x = c("ANON_ID","Clin_Result_Date"), by.y = c("ANON_ID","CONTACT_DATE") )
labs.vitals = labs.vitals[!is.na(labs.vitals$Pulse) & !is.na(labs.vitals$Pulse),] # TODO: Line 10 error: should be vitals$Temp?
...
for (j in 1:2){
  ...
  for (i in 1:length(top8)){
    clin = top8[i]
    #2*(i - 1) + j + 1
    #matchidx <- as.data.frame(which(layout == i, arr.ind = TRUE)) ## need to figure this line out
    ...
    if (nrow(labs.vitals.tmp) ){
      ...
      #                        layout.pos.col = matchidx$col)) ## need to figure this line out
    }
  }
  dev.off()
}
