...
#for(i in 1:3){
  ...
  subDfMorning <- with( subDf , subDf[ hour( Date ) >= 8 & hour( Date ) < 9 , ] )
  subDf <- subDfMorning
  ...
  restingDate <- restingDate[which(restingST<60 & !is.na(restingST))]
  restingMD5 <- restingMD5[which(restingST<60 & !is.na(restingST))]
  restingHR <- subDf$Heart_Rate[which(restingST<60 & !is.na(restingST))]
  restingST <- restingST[which(restingST<60 & !is.na(restingST))]
  restingSteps <- subDf$Steps[which(restingST<60 & !is.na(restingST))]
  
  
  restingDf[i,1] <- paste(restingMD5,collapse=",")
  restingDf[i,2] <- paste(restingDate,collapse=",")
  restingDf[i,3] <- paste(restingHR,collapse=",")
  restingDf[i,4] <- paste(restingSteps,collapse=",")
  restingDf[i,5] <- paste(restingST,collapse=",")
  restingDf[i,6] <- paste(restingDate,collapse=",")
}
...
restingHR_Df <- data.frame("iPOP_ID"=unlist(strsplit(restingDf$iPOP_ID,",")),
                           ...
                           "restingSteps"=unlist(strsplit(restingDf[,4],",")),
                           "restingST"=unlist(strsplit(restingDf[,5],",")),
                           "DateTime"=unlist(strsplit(restingDf$Date,",")),
                           stringsAsFactors = FALSE)

restingHR_Df[,3:5] <- sapply(restingHR_Df[,3:5], as.numeric)
...
write.csv(plotDf, paste0(dir,"/20180416/20180417_8am_resting10minThresh60HRvPulse_plot.csv"))
...
#                              header=TRUE,sep=',',stringsAsFactors=FALSE)
# 
# plotDf <-read.csv("/Users/jessilyn/Desktop/Fig1D_timecourse/20180409_3am_restingHRvPulse_plot.csv")
# library(reshape2)
# plotDf.m <- melt(plotDf,id.vars='iPOP_ID', measure.vars=c('restingHR','Pulse'))
# tt <- table(plotDf.m$iPOP_ID)
# plotDf.m.many <- subset(plotDf.m, iPOP_ID %in% names(tt[tt > 20])) # try restricting this analysis to people who have more than x clinic visits (e.g. 5 visits)
# plotDf.demo <- subset(iPOPdemographics, iPOP_ID %in% names(tt[tt > 20]))
# 
