...
patients = sort(table(corDf$ANON_ID))
...
#corDf = corDf[corDf$Pulse > 50,]
#corDf = corDf[corDf$Temp > 96.5,]
patients.GLU = sort(table(corDf[!is.na(corDf$GLU_byMeter),]$ANON_ID))
top.patients.GLU = names(patients.GLU[patients.GLU > 20])

# GLU vs vitals
png('plots/big-glu-pulse.png',width = 1200, height = 800,res=120)
print(ggplot(corDf[corDf$ANON_ID %in% top.patients.GLU,], aes(Pulse, GLU_byMeter, group = ANON_ID, colour = ANON_ID)) + 
...
print(ggplot(corDf[corDf$ANON_ID %in% top.patients.GLU,], aes(Temp, GLU_byMeter, group = ANON_ID, colour = ANON_ID)) + 
  weartals_theme + theme(text = element_text(size=20))+ theme(legend.position="none") +
...
print(ggplot(corDf[corDf$ANON_ID %in% top.patients.GLU,], aes(Pulse, PLT, group = ANON_ID, colour = ANON_ID)) + 
  weartals_theme + theme(text = element_text(size=20))+ theme(legend.position="none") +
  geom_point(size=3) + geom_smooth(method="lm", fill=NA))
dev.off()

png('plots/big-plt-temp.png',width = 1200, height = 800,res=120)
print(ggplot(corDf[corDf$ANON_ID %in% top.patients.GLU,], aes(Temp, PLT, group = ANON_ID, colour = ANON_ID)) + 
  weartals_theme + theme(text = element_text(size=20))+ theme(legend.position="none") +
  geom_jitter(size=3) + geom_smooth(method="lm", fill=NA))
dev.off()
...
data= corDf[corDf$ANON_ID %in% top.patients.GLU,]
...
df = corDf[corDf$ANON_ID %in% top.patients.GLU,]

png('plots/glu-scatter.png',width = 1200, height = 800,res=120)
toplot = corDf[,which(colnames(corDf) %in% c("GLU_byMeter","Temp","Pulse") )]
toplot = na.omit(toplot)
plot(toplot)
dev.off()

png('plots/other-scatter.png',width = 1200, height = 800,res=120)
toplot = corDf[,which(colnames(corDf) %in% c(names(top),"Temp","Pulse") )[c(5:12)]]
toplot = na.omit(toplot)
plot(toplot)
dev.off()

png('plots/other-scatter.png',width = 1200, height = 800,res=120)
toplot = corDf[,which(colnames(corDf) %in% c(c("HSCRP","PLT","RDW"),"Temp","Pulse") )]
#toplot = na.omit(toplot)
plot(toplot)
dev.off()

png('plots/other-scatter.png',width = 1200, height = 800,res=120)
toplot = corDf[,which(colnames(corDf) %in% c(c("NEUT","NEUTAB","LYM"),"Temp","Pulse")) ]
toplot = na.omit(toplot)
...
for (i in 1:length(top8)){
  for (j in 1:2){
    ...
    patients.tmp = sort(table(corDf[!is.na(corDf[[clin]]),]$ANON_ID))
    corDf.tmp = corDf[corDf$ANON_ID %in% names(patients.tmp[patients.tmp > min_visits]),]
    if (nrow(corDf.tmp) > 2){
      frm = paste0(clin," ~ ",vit," + (",vit,"|ANON_ID)")
      print(frm)
      mm = lmer(frm, data = corDf.tmp)
      cf = coef(mm)
      png(paste0("plots/30K-",vit,"-",clin,".png"))
      qq = qplot(cf$ANON_ID[vit], geom="histogram")  + weartals_theme + xlab(paste0(names(nms)[i]," ~ ",vit)) + ylab("count")
      print(qq)
      dev.off()
    }
  }
...
for (i in 1:length(8)){
    ...
    corDf.tmp = corDf[corDf$ANON_ID %in% names(patients.tmp[patients.tmp > min_visits]),]
    if (nrow(corDf.tmp) > 2){
      corDf.tmp$ANON_ID = factor(corDf.tmp$ANON_ID)
      ...
      print(frm)
      mm = lmer(frm, data = corDf.tmp[train,])
      cf = coef(mm)
      vit = "Pulse"
      ...
      m0 = lm(frm, corDf.tmp[train,])
      pp = predict(m0, newdata = corDf.tmp[test,])
      plot(pp, tt)
      print(cor(pp,tt,use = "na.or.complete"))
      
      # Evaluate MM model
      tt = corDf.tmp[test,clin]
      pp = predict(mm, newdata = corDf.tmp[test,])
      plot(pp, tt)
      print(cor(pp,tt,use = "na.or.complete"))
...
corDf.tmp = corDf[corDf$ANON_ID %in% names(patients[patients > 20]),]
...
dd = corDf[corDf$ANON_ID=="N-7881",c(vit,clin)]
...
dd = corDf[corDf$ANON_ID=="D-6050",c(vit,clin)]
ggplot(dd, aes_string(x = clin, y = vit)) + weartals_theme + 
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1.5)
...
for (iid in c(1:3)){
  vit = "Temp"
  clin = "NEUT"
  dd = corDf[corDf$ANON_ID %in% c(toppat[iid]) ,c("ANON_ID",vit,clin)]
...
ggplot(corDf, aes(x = NEUT, y = Pulse)) + weartals_theme + 
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1.5)
ggsave("plots/quadratic-neut.png")
ggplot(corDf, aes(x = LYM, y = Pulse)) + weartals_theme + 
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1.5)
ggsave("plots/quadratic-lym.png")
ggplot(corDf, aes(x = GLU_fasting, y = Pulse)) + weartals_theme + 
  geom_point() +
  stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1.5)
