...
# check dates
max(unique(as.Date(vitals$CONTACT_DATE, "%d-%b-%Y")))
head(sort(unique(as.Date(vitals$CONTACT_DATE, "%d-%b-%Y")), decreasing = FALSE), n=20)
max(unique(as.Date(labs$TAKEN_DATE, "%d-%b-%Y")))
head(sort(unique(as.Date(labs$TAKEN_DATE, "%d-%b-%Y")), decreasing = TRUE), n=20)
# there are a few errors in dates in the labs$TAKEN_DATE but when we merge with the labs data those wrong dates should get removed anyways 

# check lab tests:
unique(labs1$LAB_NAME)[grep("phosphate", (unique(labs1$LAB_NAME)), ignore.case = TRUE)] # does this lab test exist in our 30k data?
# to add: "Procalcitonin" and "PROCALCITONIN" (1532 measures on 791 unique people - not sure how many correspond to vital signs)
#  "TROPONIN I" "Troponin I" "Troponin I, POCT" (5724 people, 36,832 measures)
# maybe include Troponin T (9697 measures on 2810 people)
# SED RATE (4886 measurements, 1835 people) ESR (grep all; 33872 measures on 9026 people)
# phosphate
length(grep("PCT", labs1$LAB_NAME, ignore.case = TRUE)) # number of measures of that test
length(unique(labs1$ANON_ID[grep("PCT", labs1$LAB_NAME, ignore.case = TRUE)])) # unique people that have that measure
...
#### NOTE WANTED LAB TESTS:
 allClin <- c("A1C","AG","ALB","ALCRU","ALKP","ALT","AST","BASO",
             ...
             "NEUTAB","NHDL","PLT","PROCALCITONIN", "RBC","RDW","TBIL","TGL","TP","TroponinI",
             "UALB","UALBCR","WBC")
...
#  "ESR"
upBound <- 70
ESR <- which(labs$LAB_NAME %in% unqLabs[
  grep("SED RATE|ESR",unqLabs)] & (as.numeric(labs$ORD_VALUE) < upBound))
test <- labs[c(ESR),]
summary(as.numeric(test$ORD_VALUE)) # summary of ESR values from 30K cohort
...
#  "PROCALCITONIN" # need to go back and add to WEAR table and fix bounds that way
upBound <- 30
PROCALCITONIN <- which(labs$LAB_NAME %in% unqLabs[
  grep("PROCALCITONIN|Procalcitonin",unqLabs)] & (as.numeric(labs$ORD_VALUE) < upBound))
test <- labs[c(PROCALCITONIN),]
summary(as.numeric(test$ORD_VALUE)) # summary of PROCALCITONIN values from 30K cohort
...
#  "TroponinI"
upBound <- 70
unique(labs[c(TroponinI),]$ORD_VALUE[grep("<", unique(labs[c(TroponinI),]$ORD_VALUE))])
labs[c(TroponinI),] <- gsub("[<]","",labs[c(TroponinI),])
#replace ">40.000",  ">40.1" , ">40.0", "<0.31" "<0.01", "<0.1" 
TroponinI <- which(labs$LAB_NAME %in% unqLabs[
  grep("TROPONIN I|Troponin I",unqLabs)])
test <- labs[c(TroponinI),]
test$ORD_VALUE <- gsub("[<]","",test$ORD_VALUE)
test$ORD_VALUE <- gsub("[>]","",test$ORD_VALUE)
unique(test$ORD_VALUE)[grep(">", unique(test$ORD_VALUE))]
summary(as.numeric(test$ORD_VALUE)) # summary of ESR values from 30K cohort
...
#                  "SECURE_lab_results_20180907_subset.csv"),
#           row.names=FALSE)

#Note: IGM, UALBCR, and UALBCRINTP are left out (not found in new labs)

#### PREP LABS DATA ####
rm(list=setdiff(ls(), c("vitals","wear")))
require(data.table)
require(psych)
labs <- fread("/Users/jessilyn/Documents/Career_Development/Mentoring/RyanRunge/20170803_FINAL_LASSOS/SECURE_lab_results_20180907_subset.csv",
      header=TRUE,sep=',',stringsAsFactors=FALSE)
#check dates
#max(unique(as.Date(labs$TAKEN_DATE, "%d-%b-%Y")))
#head(sort(unique(as.Date(labs$TAKEN_DATE, "%d-%b-%Y")), decreasing = TRUE), n=20)
...
# check dates
#head(sort(unique(as.Date(labs$Clin_Result_Date, "%Y-%m-%d")), decreasing = TRUE), n=20)
...
require(reshape2)
labs2<-labs[,c(1,4,7,8)]
str(labs2)
timer=proc.time()
aqw <- dcast(labs2, ANON_ID + Clin_Result_Date ~ LAB_NAME , fun.aggregate = mean)
proc.time() - timer
# write.csv(aqw,
#           paste0("/Users/jessilyn/Documents/Career_Development/Mentoring/RyanRunge/20170803_FINAL_LASSOS/",
#                  "20170907_test.csv"),
...
# check file
labs2[which(labs2$ANON_ID %in% "D-1" & labs2$Clin_Result_Date %in% "1999-07-01")]

##### PROBLEM HERE: corDF not yet defined
head(corDf)[1,]

vitalVars <- which(names(corDf) %in% c("Pulse","Temp"))

#Change labs data to wide format - this code crashed R once the dates were fixed, so I fixed this (see above)
...
vitals$PULSE [(vitals$PULSE > 230) | (vitals$PULSE < 20) ] <- NA; length(na.omit(vitals$PULSE)) # remove values outside of phys range
vitals$TEMPERATURE [(vitals$TEMPERATURE > 115) | (vitals$TEMPERATURE < 90) ] <- NA; length(na.omit(vitals$TEMPERATURE)) # remove values outside of phys range
vitals$BP_SYSTOLIC [(vitals$BP_SYSTOLIC > 220) | (vitals$BP_SYSTOLIC < 70) ] <- NA; length(na.omit(vitals$BP_SYSTOLIC))
vitals$BP_DIASTOLIC [(vitals$BP_DIASTOLIC > 130) | (vitals$BP_DIASTOLIC < 35) ] <- NA; length(na.omit(vitals$BP_DIASTOLIC))
vitals$RESPIRATIONS [(vitals$RESPIRATIONS > 130) | (vitals$RESPIRATIONS < 2) ] <- NA; length(na.omit(vitals$RESPIRATIONS))
...
hist(vitals$Pulse, breaks=100)

# add in additional vitals that we can't measure from the watch to improve the model, and to make the case for additional types of wearables
#vitals$HEIGHT <- gsub("['\"']","",vitals$HEIGHT)
#vitals$HEIGHT <- as.numeric(vitals$HEIGHT); vitals$WEIGHT <- as.numeric(vitals$WEIGHT)
#vitals$BMI <- vitals$WEIGHT / (vitals$HEIGHT)^2
...
tot <- 0
for (i in 7:56){
tmp <- length(as.matrix(na.omit(wear[i])))
tot <- tot + tmp
} tot
...
require(cowplot)
topHits <- c("GLU_nonFasting","CR","NEUTAB","NEUT","HSCRP","LYM", "RDW", "ALB","AG", "PLT", "TroponinI", "ESR", "PROCALCITONIN")

df2<-corDf[names(corDf) %in% topHits]


df <- cbind(corDf[(names(corDf) %in% topHits)], corDf$Pulse, corDf$Temp)
plot(df, pch=19, cex=0.2)

j=0
plots<-c()
for (i in topHits){
  p<- ggplot(corDf, aes(x=corDf[[i]], y=corDf$Pulse))
  print(p)
  j=j+1}
...
plot_grid(plots[1:j], labels = topHits)
