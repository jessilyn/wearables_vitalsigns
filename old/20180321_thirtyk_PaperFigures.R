...
# model uses 10% of people as test set in LOO CV ; I just added an outer loop for Monte Carlo CV aka repeated random subsampling validation
# e.g. as a measure og accuracy/ predictive error by determining the variability in the correlations between observed and predicted
corr.coefs.lm<-c()
corr.coefs.quad<-c()
corr.coefs.rf<-c()
for (i in 1:50){
...
# runs friday night to monday morning
for (mode in modes){
  ...
  for (k in 1:length(patients)){
    ...
    for (l in 1:length(top.names)){
      ...
      x.train<-x.train[,colnames(x.train) %in% c(top.names[l], wear.variables, "iPOP_ID")] # subset input data by lab: only take current lab test of interest
      ...
      if(mode == "lasso"){
        #frq <- as.vector(table(as.character(x.train$iPOP_ID))) #decide on number for nfolds from number of obs per subject
        # lasso 
        n <- as.numeric(length(outcome)) #optional argument for leave-one-out CV method
...
#write.table(num.Records, "../SECURE_data/20180326_num_Records_week_prior.csv",row.names=FALSE,col.names=FALSE, sep=",")
...
data <-read.table("../SECURE_data/20180330/20180327/20180327_corr_coeffs_AllData.csv",
                  header=TRUE,sep=',',stringsAsFactors=FALSE)
...
#dev.off()
write.table(data, "../SECURE_data/20180326_Fig2C_one_thru_4_corr_coeffs_week_prior.csv",row.names=FALSE,col.names=FALSE, sep=",")
...
j<-"LYM"
for (j in clinTopTen){
  #print(ggplot(corDf, aes(y = corDf[[j]], x = corDf$Pulse)) + stat_binhex())
        
  print(ggplot(corDf, aes(y = corDf[[j]], x = corDf$Pulse)) +
  ...
  print(ggplot(corDf2, aes(y = bin2, x = Pulse)) +
          stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1.5, col="darkred") +
          theme(axis.title=element_text(face="bold",size="14"),axis.text=element_text(size=16,face="bold"), panel.background = element_blank(), axis.line = element_line(colour = "black"))+
