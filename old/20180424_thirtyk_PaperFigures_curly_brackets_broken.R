...
### clean wear ### messes up code for Fig 2C
# wear2<-wear
...
for (mode in modes){
  ...
  for (k in 1:5){
    ...
    res.nullmod.pred = list(lm=list(),rf=list())
    p.value<-list()
    ...
    for (l in 1:length(top.names)){
      ...
      #fml.null = paste("cbind(",paste(top.names[l],collapse=" , "),") ~ 1") # check with Trevor that this is the correct Null model - it sets the intercept equal to mean(outcome) so I think it is right
      fml.null = paste(top.names[l]," ~ 1") # check with Trevor that this is the correct Null model - it sets the intercept equal to mean(outcome) so I think it is right
      set.seed(1)
  ...
  # Get RSSm and RSS0 to calculate % variance explained (aka %  deviance explained )
  # for (mdl.name in model.names){
  #   rssm.wear = c()
  #   rss0.wear = c()
  #   pct.dev.explained = c()
  #   for (j in 1:length(top.names))
  #     rssm.wear = c(rssm.wear, sum((val.pred[[mdl.name]][[j]] - na.omit(val.true[[j]]))^2))
  #     rss0.wear = c(rss0.wear, sum((val.nullmod.pred[[mdl.name]][[j]] - na.omit(val.true[[j]]))^2))
  #     pct.dev.explained = ( rss0.wear - rssm.wear ) / rss0.wear
  #   names(pct.dev.explained) = top.names ; names(rss0.wear) = top.names
...
#                   header=TRUE,sep=',',stringsAsFactors=FALSE)
data <-read.table("/Users/jessilyn/Desktop/framework_timecourse/with_resting_bugfix_and_demographics/20180420_corr_coeffs_DayPrior_demog.csv",
                  header=TRUE,sep=',',stringsAsFactors=FALSE)
...
# use iPOPcorDf.demo as the data source - it is cleaned & outliers are removed
